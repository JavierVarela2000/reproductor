...
class Player {
  ...
  constructor(audio_player = null) {
    if (audio_player) {
      ...
      if (typeof window === "undefined") console.warn("El entorno no tiene acceso a window. El reproductor de audio no se inicializará.");
  ...
  next() {
    ...
    const currentIndex = this.current_playlist.tracks.findIndex((track) => track === this.current_track);
  ...
  prev() {
    ...
    const currentIndex = this.current_playlist.tracks.findIndex((track) => track === this.current_track);
    const prevIndex = (currentIndex - 1 + this.current_playlist.tracks.length) % this.current_playlist.tracks.length;
